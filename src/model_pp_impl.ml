...
module Make (Spec : SPEC) = struct
  ...
  module CIDataPP = Algdt_pp.Make_IData (struct
  ...
  and pp_vprod_els tps n ppf vprod_els =
    ...
    let coll n vprod_el =
      fprintf ppf ",@ ";
      pp_vprod_el tps n ppf vprod_el
    in
  ...
  and pp_dvar_mod tps n ppf = function
    | DVarStrct (tp, cnstr, vprod_el) ->
        ...
        new_n
    | DVarFree dsum ->
        CIDataPP.pp_sum ppf dsum;
        n
    | DVar tp ->
        fprintf ppf "%s_%s%d" ctp_tbl.(tp) csuf n;
        n + 1
  ...
  let pp_dmodel ppf dmodel ddpat =
    ...
    let rec pp_dmodel ppf = function
      | DVal deco_sums ->
          if Array.length deco_sums = 1 then
            CIDataPP.pp_data_el ppf deco_sums.(0)
          else CIDataPP.pp_prod_els ppf deco_sums
      | DLet (dmatch_mod, tps, vprod_els) ->
          fprintf ppf "@[<2>@[let@ %a@ =@]@\n@[%a@]@]@ in@\n@[%a@]" pp_vars tps
            pp_dmatch_mod dmatch_mod (pp_vprod_els_start tps) vprod_els
    ...
    and pp_dmatch_mod ppf dmatch_mod =
      ...
      | DShave (var_ix, tp, cnstr, pat, dsh_mod, dmodel) ->
          ...
          fprintf ppf "@\n@[<4>| @[%t@]%a@]" (pp_match tp cnstr pat) pp_branch
            dsh_mod;
