...
let pp_open_par_box ppf () =
  pp_print_char ppf '(';
  pp_open_box ppf 0

let pp_close_par_box ppf () =
  pp_close_box ppf ();
  pp_print_char ppf ')'
...
module Make_Data (Spec : DATA_PP_SPEC) = struct
  ...
  and pp_sum_ is_arg ppf = function
    ...
    | DStrct (tag, arg) ->
        fprintf ppf "%a@[<2>%a@ %a@]%a" pp_par_left is_arg Spec.pp_tag tag
          pp_arg arg pp_par_right is_arg
...
module Make_IData2
    ...
    (CIData_Meta_Spec : IDATA_META_SPEC) =
struct
