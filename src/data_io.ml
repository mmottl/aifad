...
(* $Id: data_io.ml,v 1.13 2006/01/17 00:23:37 mottl Exp $ *)
...
let store_node node = function
  | (i, ar) :: rest ->
      ar.(i) <- node;
      let i' = i + 1 in
      if i' = Array.length ar then rest
      else (i', ar) :: rest
  | [] -> assert false
...
let make_cnv_data { cnstr_htbl; ispec; tp_tbl; _ } fspec =
  let find_cnstr tp cnstr =
    ...
    with Not_found ->
      let err =
        Printf.sprintf "find_cnstr: unknown constructor %s of type %s"
          cnstr tp_tbl.(tp) in
      failwith err in
  ...
  and cnv_data ar_lst pe dpe =
    ...
    | ProdEls _, TpVal _ -> failwith "cnv_data: expected product" in
  cnv_data_rhs
