...
let report (cfspec, ctps) read_eval read =
  ...
  let rec loop all_bits cmn_bits avg_sum n_samples =
    ...
    | Some eval_fdsums, Some fdsums ->
        let cmn =
          calc_cmn_fdsums_bits cfspec_bits cfspec ctps eval_fdsums fdsums
        in
    ...
    | _ -> failwith "report: datasets have different size!"
  in
  ...
  printf
    "Samples:              %d@\n\
     ...
     Bit Accuracy:         %f@\n\
     Sample accuracy:      %f@\n"
    n_samples all_bits (all_bits /. f_samples) cmn_bits (cmn_bits /. f_samples)
    (cmn_bits /. all_bits) (avg_sum /. f_samples)
